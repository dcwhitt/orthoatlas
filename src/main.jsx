    ...
    const tick = () => {
      ...
      if (video && canvas && models && video.readyState >= 2) {
        ...
        if (recordingRef.current && detection) {
          ...
          const measured = computeFrameAngles(frame, { selectedJoint: session.joint, selectedSide: session.affectedSide, minConfidence: 0.7 });
          if (Object.keys(measured.angles).length > 0) {
...
function ResultsCard({ results, session, selectedJoint, onCopy }) {
  ...
  const rows = sides.map(side => selectedRomRow(results?.[side], session.joint, side)).filter(Boolean);
  ...
  return <section className="card">
    ...
    <div className="dominant-result"><span>{selectedJoint.label}</span><strong>{hasResults ? 'ROM measured' : '--'}</strong></div>
    <div className="selected-rom-table">
      {rows.length ? rows.map(row => <div className="rom-row" key={row.side}>
        <h3>{capitalize(row.side)} {selectedJoint.label}</h3>
        {row.measured ? <div className="rom-values">{row.values.map(item => <Metric key={item.label} label={item.label} value={item.value} />)}</div> : <p className="mini-status">No high-confidence ROM values recorded yet for this side/joint.</p>}
...
function selectedRomRow(sideResults, joint, side) {
  ...
  if (joint === 'ankle') values.push({ label: 'Dorsiflexion/plantarflexion min', value: deg(s.ankle?.ankle_min_deg) }, { label: 'Dorsiflexion/plantarflexion max', value: deg(s.ankle?.ankle_max_deg) }, { label: 'Arc', value: deg(s.ankle?.arc_deg) });
  return { side, values, measured: values.some(v => v.value !== '--') };
}
...
function romNote(session, results) {
  ...
  const parts = sides.map(side => {
    const row = selectedRomRow(results?.[side], session.joint, side);
    if (!row?.measured) return `${capitalize(side)}: not measured`;
    return `${capitalize(side)}: ${row.values.map(v => `${v.label} ${v.value}`).join(', ')}`;
  });
